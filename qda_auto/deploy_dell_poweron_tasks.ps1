...
$LocalScript = Join-Path $Base "dell_poweron_tasks_local.bat"
$ToolFolder = Join-Path $Base "DELL_CMD"
...
if ($CopyTool -eq "Y") {
    if (!(Test-Path $ToolFolder)) {
        Write-Host "[LOI] Khong thay folder DELL_CMD"
        pause
        exit
    }

    $Cctk1 = Join-Path $ToolFolder "cctk.exe"
    $Cctk2 = Join-Path $ToolFolder "X86_64\cctk.exe"

    if (!(Test-Path $Cctk1) -and !(Test-Path $Cctk2)) {
        Write-Host "[LOI] Khong thay cctk.exe trong DELL_CMD hoac DELL_CMD\X86_64"
        pause
        exit
    }
}
...
$SuccessFile = Join-Path $Base "dell_poweron_tasks_success.txt"
$FailedFile = Join-Path $Base "dell_poweron_tasks_failed.txt"
...
foreach ($IP in $Targets) {
    ...
    $Jobs += Start-Job -ArgumentList $IP,$AdminUser,$AdminPass,$PsExec,$LocalScript,$ToolFolder,$CopyTool,$DoBios,$DoApp,$SUN,$MON,$TUE,$WED,$THU,$FRI,$SAT,$TargetTime,$AppDelay -ScriptBlock {
        ...
        try {
            ...
            $RemoteRoot = "\\$IP\C$\Windows\Temp\DELL_AUTOON"
            $RemoteTool = "\\$IP\C$\Windows\Temp\DELL_AUTOON\DELL_CMD"
            $RemoteBat = "\\$IP\C$\Windows\Temp\DELL_AUTOON\dell_poweron_tasks_local.bat"

            New-Item -ItemType Directory -Path $RemoteRoot -Force | Out-Null

            Remove-Item $RemoteBat -Force -ErrorAction SilentlyContinue
            Remove-Item "\\$IP\C$\dell_poweron_tasks_result.txt", "\\$IP\C$\dell_poweron_tasks_log.txt" -Force -ErrorAction SilentlyContinue
            ...
            $TaskName = "DellPowerOnTasks"
            $RemoteLocal = "C:\Windows\Temp\DELL_AUTOON\dell_poweron_tasks_local.bat"
            $Args = "$DoBios $SUN $MON $TUE $WED $THU $FRI $SAT $TargetTime $DoApp $AppDelay"
            $RunLine = "`"$RemoteLocal`" $Args"

            & $PsExec "\\$IP" -u $AdminUser -p $AdminPass -s -h -accepteula -nobanner cmd /c "schtasks /Delete /TN $TaskName /F" 2>$null | Out-Null

            $CreateCmd = "schtasks /Create /TN $TaskName /SC ONCE /ST 23:59 /RL HIGHEST /RU $AdminUser /RP $AdminPass /TR `"cmd.exe /c $RunLine`" /F"

            & $PsExec "\\$IP" -u $AdminUser -p $AdminPass -s -h -accepteula -nobanner cmd /c $CreateCmd 2>$null
            ...
            & $PsExec "\\$IP" -u $AdminUser -p $AdminPass -s -h -accepteula -nobanner cmd /c "schtasks /Run /TN $TaskName" 2>$null
            ...
            if ($LASTEXITCODE -ne 0) {
                ...
                return [PSCustomObject]@{
                    IP = $IP
                    Status = "FAILED"
                    Message = "RUN TASK FAIL"
                }
            }

            $ResultPath = "\\$IP\C$\dell_poweron_tasks_result.txt"
            $LogPath = "\\$IP\C$\dell_poweron_tasks_log.txt"
            ...
            Remove-Item "\\$IP\C$\dell_poweron_tasks_log.txt" -Force -ErrorAction SilentlyContinue
            Remove-Item "\\$IP\C$\dell_poweron_tasks_result.txt" -Force -ErrorAction SilentlyContinue
...
Write-Host ""
Write-Host "=========================================="
Write-Host "        KET QUA DELL POWER-ON TASKS"
Write-Host "=========================================="
